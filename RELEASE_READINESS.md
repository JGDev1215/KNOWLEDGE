...
This public-domain subset is the only mode that can be treated as release-cleared from the current repository evidence. `RELEASE_GATE.md` records the release gate and warns that `npm run build` is a full local-study build, not a release-approved artifact.
