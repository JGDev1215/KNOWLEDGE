...
| --- | --- | --- | --- |
| Lecture-derived opinions presented as facts | Medium | Lecture-derived pages now carry app-level provenance, local-only release labels, separate library grouping, in-page audit notes, study-card source footers, and uncertified source-check drill prompts. Broad claims remain uncited but are controlled as local-only interpretation in `CLAIM_CITATION_BACKLOG.md`. | Keep as lecture interpretation, remove/reword, or add scholarly citations before treating as fact. |
...
The full app is technically usable, has repeatable audit checks, and warns users about provenance and interpretation risk at point of use and inside lecture-derived or unsupported source pages. The full app is explicitly not public-release ready, and `RIGHTS_CLEARANCE.md` records the unresolved rights evidence. `CLAIM_CITATION_BACKLOG.md` now separates open source/provenance blockers from controlled local-only interpretation. `COMPLETION_AUDIT.md` records the full-exercise completion gate. A separate public-domain-only build is now available and audited. The remaining work is rights confirmation, missing source provenance, and scholarly citation if lecture-derived claims need to be promoted from interpretation to fact.
